...
async def get_documents(
    ...
    predicate = _generate_query_predicate(filters)
    logger.debug(f"Mongo query predicate for collection '{collection_name}': {predicate}")
    ...
    logger.debug(
        "get_documents: collection='{}' filters='{}' returned {} documents".format(
            collection_name, filters, len(cleaned_results)
        )
    )
...
def upsert_data_sync(collection_name: str, docs: list[dict], batch_size: int = 50_000):
    ...
    for i in range(0, len(docs), batch_size):
        ...
        operations = []
        for doc in batch:
            # Avoid modifying immutable `_id` during an update. Use an update
            # with $set for all fields except `_id`, and $setOnInsert to set
            # `_id` only when inserting a new document.
            doc_copy = dict(doc)
            id_val = doc_copy.pop("_id", None)
            key = doc_copy.get("_key")
            # Prepare the update document
            update_doc = {"$set": doc_copy}
            if id_val is not None:
                update_doc["$setOnInsert"] = {"_id": id_val}

            operations.append(
                UpdateOne({"_key": key}, update_doc, upsert=True)
            )
